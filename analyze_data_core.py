# ==============================================================================
# File: analyze_data_core.py (Core Analysis Logic)
# ==============================================================================
import pandas as pd
import xlsxwriter
import warnings
...
def analyze_data_core(raw_data_stream, fp_stream):
    """
    Performs the core data analysis logic.
    Args:
        raw_data_stream (io.BytesIO): The Excel file as a byte stream.
        fp_stream (io.BytesIO): The false positive file as a byte stream.
    Returns:
    ...
    try:
        ...
        # Load the false positive list from the stream
        fp_list = []
        if fp_stream:
            fp_list = [int(line.strip()) for line in fp_stream.read().decode('utf-8').splitlines() if line.strip()]
            print(f"Loaded {len(fp_list)} false positives for filtering.")

        # --- Your original logic from here on ---
        column_mapping = {
